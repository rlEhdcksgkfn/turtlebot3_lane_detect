...
class ArucoMarkerListener(Node, ArucoStateMachine):
        ...
        for marker in msg.markers:
            if marker.id == self.target_marker_id:
                ...
                self.get_logger().info(
                    f"[{time.strftime('%H:%M:%S')}] Marker ID: {marker.id}, "
                    f"Position: x={self.aruco_position['x']}, y={self.aruco_position['y']}, z={self.aruco_position['z']}"
                )
                self.marker_detected()

                # 바퀴 속도 제어
                if self.state == 'MOVING_TO_MARKER':
                    self.aruco_position['z'] < 0.21
    ...
    def aruco_arm_controll(self):
        ...
        self.get_logger().info(
            f"[{time.strftime('%H:%M:%S')}] Mission ArUco marker coordinates: "
            f"x={self.aruco_position['x']}, y={self.aruco_position['y']}, corrected_z={corrected_z}"
        )
