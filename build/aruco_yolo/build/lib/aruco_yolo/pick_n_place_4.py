...
class ArucoMarkerListener(Node, ArucoStateMachine):
    def __init__(self):
        ...
        self.last_marker_visible = False  # 이전 마커 감지 상태
    ...
    def aruco_listener_callback(self, msg):
        ...
        for marker in msg.markers:
            if marker.id == self.target_marker_id:
                ...
                # 거리 계산
                distance = math.sqrt(
                    self.aruco_position['x'] ** 2 +
                    self.aruco_position['y'] ** 2 +
                    self.aruco_position['z'] ** 2
                )
                self.get_logger().info(f"[{time.strftime('%H:%M:%S')}] 📏 마커 거리: {distance:.3f}m")

                # 예: 0.5m 이내일 때 정지 및 픽앤플레이스 시작
                if distance < 0.5 and not self.armrun:
                    self.get_logger().info(f"[{time.strftime('%H:%M:%S')}] ✅ 거리 기준 충족 → 자율주행 정지 및 픽앤플레이스 시작")
                    self.autonomous_enable_pub.publish(Bool(data=False))
                    self.last_marker_visible = True
                    # self.aruco_arm_controll()

                break  # 첫 번째 감지된 마커만 처리

        # 마커가 없고 이전엔 있었을 경우 → 재개하지 않음 (픽 완료로 판단)
