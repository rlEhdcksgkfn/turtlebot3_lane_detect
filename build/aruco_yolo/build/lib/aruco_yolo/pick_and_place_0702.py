...
##################################################################################
    ...
    def aruco_arm_controll(self):
        ...
        # 초기 관절 위치로 이동 (항상 초기 위치 유지)
        print("Move to Initial Position")
        self.point.positions = [0.17, -0.058, -0.258, 1.94]
        self.get_logger().info(f"Publishing initial joint positions: {self.point.positions}")
        self.joint_pub.publish(self.trajectory_msg)

        # ArUco 마커 감지 대기
        while rclpy.ok() and not self.aruco_marker_found:
            self.get_logger().warn("No ArUco marker detected. Staying at initial position.")
            rclpy.spin_once(self, timeout_sec=0.5)  # 주기적으로 마커 감지 확인

        # 마커가 감지된 경우
        # z 위치 보정: y = 2.09x - 5.55
        corrected_z = 2.09 * self.aruco_position_z - 5.55
        self.get_logger().info(f"Mission ArUco marker Location coordinates: x: {self.aruco_position_x}, y: {self.aruco_position_y}, corrected_z: {corrected_z}")
        self.armrun = True
        ...
        for cmd, posename, description in moveit_steps:
            ...
            if response.response:
                ...
                self.get_logger().error(f"Failed: {description}")
                self.armrun = False
                return
            rclpy.spin_once(self, timeout_sec=1.0)

        # ArUco 마커 위치로 이동 (보정된 z값 사용)
        print("Moving to target position")
        pose_array = self.append_pose_init(
            self.aruco_position_x,
            self.aruco_position_y,
            corrected_z  # 보정된 z값 사용
        )
